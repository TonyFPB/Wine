...
export async function recuperarAvaliacaoAcimaDaNota() {
  const vinhos = await connection.query(`
    select u.usuario_id, u.nome as usuario_nome,v.vinho_id, v.nome as vinho, a.comentario, a.nota 
    from Usuario u 
...
export async function recuperarQuantidadeAvaliacoes() {
  const qtdAv = await connection.query(`
    select v.vinho_id,v.nome, COUNT(v.vinho_id) as 'quantidade_de_avaliaçoes' from Usuario u 
	    join Avaliacao a ON u.usuario_id = a.fk_Usuario_usuario_id 
	    join Vinho v ON a.fk_Vinho_vinho_id = v.vinho_id
...
export async function recuperarMediaVinhosPais() {
  const mediaVinhos = await connection.query(`
    select vin.pais , AVG(a.nota) as 'media' from Usuario u 
      join Avaliacao a ON u.usuario_id = a.fk_Usuario_usuario_id 
      join Vinho v ON a.fk_Vinho_vinho_id = v.vinho_id
...
export async function recuperarUsuariosSemAvaliacoes() {
  const usuarios = await connection.query(`
    select u.usuario_id,u.nome from Usuario u 
...
export async function recuperarVinhosMaiorNota() {
  const vinhos = await connection.query(`
    select distinct v.vinho_id, vin.vinicola_id, vin.nome as vinicola, v.nome, YEAR(v.safra) as safra , vin.pais, a.nota
      from Vinho v
...
export async function recuperarVinhosAvaliadosPorUsuariosExperientes() {
  const vinhos = await connection.query(`
    select distinct v.nome, vin.nome 
      from Vinho v
      join Avaliacao a on v.vinho_id = a.fk_Vinho_vinho_id
    ...
    where a.nota > 80
      and u.usuario_id in (
        select a2.fk_Usuario_usuario_id
        from Avaliacao a2
        group by a2.fk_Usuario_usuario_id
        having COUNT(*) > 8000
      ) limit 10;`
  )
  return vinhos[0];
